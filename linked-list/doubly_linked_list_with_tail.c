...
int main() {
    ...
    for (int i = 0; i < 5; ++i) {
        insert_node_at_head(&head, &tail, create_new_node(i+1));
    }
    print_nodes(head, "Initialize a list");

    node_t *node_to_insert_after = find_node(head, 2);
    insert_node_after_node(&node_to_insert_after, create_new_node(15), &tail);
    print_nodes(head, "Inserting a node after 2");

    insert_node_at_tail(&head, &tail, create_new_node(100));
    print_nodes(head, "Inserting a node at the tail");

    delete_node(&head, &tail, find_node(head, 4));
    print_nodes(head, "Deleting the node with value 4");

    delete_node(&head, &tail, find_node(head, 5));
    print_nodes(head, "Deleting the head");

    delete_node(&head, &tail, find_node(head, 100));
    print_nodes(head, "Deleting the tail");

    node_t *foundNode = find_node(head, 1);
    insert_node_after_node(&foundNode, create_new_node(22), &tail);
    print_nodes(head, "Inserting a node after the tail");

    insert_node_after_node(&head, create_new_node(45), &tail);
